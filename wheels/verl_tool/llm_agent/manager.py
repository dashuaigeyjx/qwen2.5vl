...
class AgentActorManager:
    def __init__(
        ...
        print(f"[DEBUG] ===== Enter AgentActorManager.__init__ =====")
        print(f"[DEBUG] model_path: {model_path}")
        print(f"[DEBUG] rollout mode: {config.rollout_mode}")
        print(f"[DEBUG] assistant_role BEFORE INIT: {config.assistant_role}")
        print(f"[DEBUG] turn_end_token BEFORE INIT: {config.turn_end_token}")
        ...
        print(f"[DEBUG] eos_token_id set to: {self.eos_token_id}")
        ...
        print(f"[DEBUG] TensorHelper initialized with pad_token_id={self.tokenizer.pad_token_id}")

        # === Action stop tokens ===
        if self.config.action_stop_tokens is not None:
            print(f"[DEBUG] action_stop_tokens path: {self.config.action_stop_tokens}")
            if os.path.exists(self.config.action_stop_tokens):
                ...
                print(f"[DEBUG] Loaded action_stop_tokens: {self.action_stop_tokens}")
            else:
                raise ValueError(f"action_stop_tokens file not found: {self.config.action_stop_tokens}")
        else:
            self.action_stop_tokens = []
            print(f"[DEBUG] No action_stop_tokens configured, using empty list.")

        # === EOS token IDs ===
        self.additional_eos_token_ids = self.config.additional_eos_token_ids
        print(f"[DEBUG] Raw additional_eos_token_ids: {self.additional_eos_token_ids}")
        ...
        print(f"[DEBUG] Final additional_eos_token_ids: {self.additional_eos_token_ids}")

        # === MTRL Separator ===
        if self.config.mtrl_sep is None:
            print(f"[DEBUG] Generating default mtrl_sep ...")
        ...
        print(f"[DEBUG] mtrl_sep set to: {self.config.mtrl_sep}")
        ...
        print(f"[DEBUG] max_model_len: {self.max_model_len}, max_action_length: {self.max_action_length}")
        ...
        if self.processor:
            self.mm_prefix, self.mm_postfix = self.processor.apply_chat_template(
                ...
                tokenize=False, add_generation_prompt=False).split("|||")
            print(f"[DEBUG] mm_prefix & mm_postfix extracted")
        else:
            self.mm_prefix = ""
            self.mm_postfix = ""
        ...
        print(f"[DEBUG] ===== INIT COMPLETE =====")
        print(f"[DEBUG] assistant_role AFTER INIT: {self.config.assistant_role}")
        print(f"[DEBUG] turn_end_token AFTER INIT: {self.config.turn_end_token}")
        print(f"[DEBUG] ================================================")


    @classmethod
    def from_rollout_config(cls, actor_rollout_wg, rollout_config, rollout_mode="async"):
        print("[DEBUG] rollout_config type:", type(rollout_config))
        print("[DEBUG] rollout_config keys:", list(rollout_config.keys()))
        if hasattr(rollout_config, '_metadata') and hasattr(rollout_config._metadata, 'source_list'):
            print("[DEBUG] rollout_config loaded from:", rollout_config._metadata.source_list)
        print(f"[DEBUG] ===== Enter from_rollout_config() =====")
        agent_config = AgentActorConfig()
        print(f"[DEBUG] Initial AgentActorConfig: assistant_role={agent_config.assistant_role}, turn_end_token={agent_config.turn_end_token}")

        rollout_agent_keys = getattr(rollout_config, 'agent', {}).keys()
        print(f"[DEBUG] rollout_config.agent keys: {list(rollout_agent_keys)}")

        for key in rollout_agent_keys:
            if key in agent_config.__dict__.keys():
                old_value = getattr(agent_config, key)
                new_value = rollout_config.agent[key]
                setattr(agent_config, key, new_value)
                print(f"[DEBUG] Overriding config key='{key}': '{old_value}' -> '{new_value}'")
        ...
        print(f"[DEBUG] After applying rollout_config:")
        print(f"         assistant_role={agent_config.assistant_role}")
        print(f"         turn_end_token={agent_config.turn_end_token}")
        print(f"         rollout_mode={agent_config.rollout_mode}")
        print(f"         tool_server_url={agent_config.tool_server_url}")

        model_path = rollout_config.model.path
        print(f"[DEBUG] model_path from rollout_config: {model_path}")


        print(f"[DEBUG] Final enforced assistant_role={agent_config.assistant_role}")
        print(f"[DEBUG] Final enforced turn_end_token={agent_config.turn_end_token}")

        print(f"[DEBUG] ===== Instantiating AgentActorManager =====")
        agent_actor_manager = cls(model_path, actor_rollout_wg, agent_config)
        print(f"[DEBUG] ===== from_rollout_config COMPLETE =====")
        print(f"[DEBUG] Final Manager Config:")
        print(f"         assistant_role={agent_actor_manager.config.assistant_role}")
        print(f"         turn_end_token={agent_actor_manager.config.turn_end_token}")
        print(f"         rollout_mode={agent_actor_manager.config.rollout_mode}")
        print(f"[DEBUG] ================================================")
    ...
    def repeat_inputs_by_n(self, inputs: DataProto):
        ...
        for i in range(ori_len):
            for j in range(n):
                ...
                inputs.non_tensor_batch['traj_ids'][i*n+j] += f"_{uuid.uuid4().hex[:8]}"
